...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
        m = (x_test - x_train[i:i+1])
        m = torch.sum(m ** 2, dim=[1,2,3])
        dists[i] = m
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # 计算平方和（sum(x²) 和 sum(y²)）
    x_train_sq = (x_train_flat ** 2).sum(dim=1, keepdim=True)  # shape: (num_train, 1)
    x_test_sq = (x_test_flat ** 2).sum(dim=1)                  # shape: (num_test,)

    # 计算矩阵乘法（x_train @ x_test.T）
    xy = x_train_flat @ x_test_flat.T  # shape: (num_train, num_test)

    # 组合结果（利用广播）
    dists = x_train_sq + x_test_sq - 2 * xy
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _,index = dists.topk(largest=False, dim=0, k=k)
    l = y_train[index]
    # print(l)
    y_pred,_ = torch.mode(l,dim=0)
    # for i in num_test:
        
    # y_pred = nums[count.max()][0]
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        knn = compute_distances_no_loops(self.x_train, x_test)
        y_test_pred = predict_labels(knn, k=k, y_train=self.y_train)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for j in k_choices:
        ...
        for i in range(num_folds):
            x_train_tmp = torch.cat((*x_train_folds[0:i],  *x_train_folds[i+1:]),dim=0)
            # print(len(x_train_folds[0:i]) + len(x_train_folds[i+1:]))
            x_val = x_train_folds[i]
            y_train_tmp = torch.cat((y_train_folds[0:i] + y_train_folds[i+1:]),dim=0)
            y_val = y_train_folds[i]
            knn = KnnClassifier(x_train_tmp, y_train_tmp)
            result = knn.check_accuracy(x_val, y_val,quiet=True,k=j)
            cur_k_acc.append(result)
        k_to_accuracies[j] = cur_k_acc
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    mean_val = []
    for i in k_to_accuracies.keys():
        mean_val.append(sum(k_to_accuracies[i]))
    best_k = list(k_to_accuracies.keys())[mean_val.index(max(mean_val))]
    # print(list(k_to_accuracies.keys()))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
